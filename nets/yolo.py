...
from nets.backbone import Backbone, Multi_Concat_Block, Conv, SiLU, Transition_Block, autopad,CBAM,SimAM,CBAMLayer
#cbam now
...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, pretrained=False):
        ...
        self.cbam_p3=CBAMLayer(transition_channels * 4,spatial_kernel=3)
        self.cbam_p4=CBAMLayer(transition_channels * 4,spatial_kernel=3)
        self.cbam_p5=CBAMLayer(transition_channels * 8,spatial_kernel=3)
    ...
    def forward(self, x):
        ...
        P5_conv=self.cbam_p5(P5_conv) #cbam
        ...
        P4_conv = self.cbam_p4(P4_conv) #cbam
        
        # 40, 40, 128 => 80, 80, 128
        P4_upsample = self.upsample(P4_conv)
        # 80, 80, 128 cat 80, 80, 128 => 80, 80, 256
        P3          = torch.cat([self.cbam_p3(self.conv_for_feat1(feat1)), P4_upsample], 1) #cbam
